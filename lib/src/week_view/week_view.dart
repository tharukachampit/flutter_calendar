...
class WeekView<T> extends StatefulWidget {
  ...
  final Function(DateTime)? onTodaySelected;
...
class WeekViewState<T> extends State<WeekView<T>> {
  ...
  void initState() {
    ...
    _bulletRadius = widget.bulletRadius!;
    if (!widget.showWeekends) {
      _weekDays
        ..remove(WeekDays.saturday)
        ..remove(WeekDays.sunday);
    }
    _arrowFromLeft = widget.arrowFromLeft;
          ...
          child: Stack(
            children: [
              Column(
                ...
                children: [
                  Padding(
                    padding: EdgeInsets.only(left: _arrowFromLeft),
                    child: _weekHeaderBuilder(
                      _currentStartDate,
                      _currentEndDate,
                    ),
                  ),
  ...
  Widget _defaultWeekPageHeaderBuilder(DateTime startDate, DateTime endDate) {
    return WeekPageHeader(
      ...
      textColor: _textColor ?? Colors.white,
      fontSize: _fontSize,
      onTitleTapped: () async {
        ...
        widget.onTodaySelected?.call(selectedDate);
